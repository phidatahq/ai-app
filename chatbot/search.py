...
st.set_page_config(page_title="Streamlit Search - Demo", page_icon=":robot:")
...
if st.button("Submit", key="generationSubmit"):
    ...
    Summary:
    """
    summary_prepped = summary_prompt.replace("SEARCH_QUERY_HERE", prompt).replace(
        "SEARCH_RESULT_HERE", result_df["result"][0]
    )
    summary = openai.Completion.create(
        engine=COMPLETIONS_MODEL, prompt=summary_prepped, max_tokens=500
    )

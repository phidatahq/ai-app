...
class Chatbot:
    ...
    def ask_assistant(self, messages):
        ...
        if "searching for answers" in assistant_response["content"].lower():
            question_extract = openai.Completion.create(
                ...
                prompt=f"Extract the user's latest question and the year for that question from this conversation: {self.conversation_history}. Extract it as a sentence stating the Question and Year",  # noqa: E501
            )
            search_result = self._get_search_results(
                question_extract["choices"][0]["text"]
            )

            # We insert an extra system prompt here to give fresh context to the Chatbot
            # on how to use the Redis results. In this instance we add it to the conversation history,
            # but in production it may be better to hide
            self.conversation_history.insert(
                -1,
                {
                    "role": "system",
                    "content": f"Answer the user's question using this content: {search_result}. If you cannot answer the question, say 'Sorry, I don't know the answer to this one'",  # noqa: E501
                },
            )
